...
7. 限界開発鯖に導入する Bot は本番環境のみに留めてください。(詳しくは[後述](#導入制限))
...
2023年8月以降、限界開発鯖 Discord に導入できる Bot には **本番環境のみ** と制限を設けています。

これは Discord においてあまりにも多数の Bot が導入されていると正しく動作しない不具合に起因しています。(詳しくは[こちら](https://zenn.dev/lis2a/articles/discord-bot-limit))

テスト実行のために Bot を導入することはしないでください。
